...
def pre_quality_check(context: AssetExecutionContext) -> str:
    ...
    version_result = subprocess.run(["fastqc", "-V"], capture_output=True, text=True)
...
def umi_adapter_trimming(context: AssetExecutionContext) -> str:
    ...
    version_result = subprocess.run(
        ["umi_tools", "--version"], capture_output=True, text=True
    )
    ...
    os.system(
        f'find {FASTQ_DATA} -name \'*.gz\' | while read -r fastq_file; do umi_tools extract --bc-pattern=NNNNCCCCNNN --stdin="$fastq_file" --stdout="{UMI_TRIMMED_DATA}/$(basename "$fastq_file")" --log="{UMI_TRIMMED_DATA}/processed.log"; done'
    )
...
def nucleotide_trimming(context: AssetExecutionContext):
    ...
    version_result = subprocess.run(["fastp", "-V"], capture_output=True, text=True)
    context.log.info(f"Fastp version: {version_result.stdout}")

    # Run Fastp on all .gz files
    os.system(
        f'find {UMI_TRIMMED_DATA} -name \'*.gz\' | while read -r fastq_file; do fastp --trim_front1 3 --length_required 20 -i "$fastq_file" -o "{TRIMMED_DATA}/$(basename "$fastq_file")" --html "{TRIMMED_DATA}/report.html" --json "{TRIMMED_DATA}/report.json" --thread 5 ; done'
    )
