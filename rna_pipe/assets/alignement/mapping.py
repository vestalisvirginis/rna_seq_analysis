...
def indexing(context: AssetExecutionContext):
    ...
    strain_indexes = {
        ...
        "bacillus_subtilis_p9b1_lys_spbeta": Path(
            REFERENCE_DIR, "p9b1_lys_spbeta.fasta"
        ),
    ...
    version_result = subprocess.run(
        ["bowtie2", "--version"], capture_output=True, text=True
    )
...
def mapping(context: AssetExecutionContext):
    ...
    mapping_command = [
        "bowtie2",
        "-x",
        f"{MAPPING_DIR}/bacillus_subtilis_p9b1_lys_spbeta",
        "-1",
        f"{TRIMMED_DATA}/spbeta_1_MKDL250000598-1A_HV7W7DSXC_L3_1.fq.gz",
        "-2",
        f"{TRIMMED_DATA}/spbeta_1_MKDL250000598-1A_HV7W7DSXC_L3_2.fq.gz",
        "-S",
        f"{MAPPING_DIR}/aligned.sam",
        "-t",
        "--threads",
        "6",
    ]
